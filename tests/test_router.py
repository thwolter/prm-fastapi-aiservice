...
def test_risk_definition_check_valid_input(mock_execute_query):
    ...
    assert isinstance(
        RiskDefinitionCheckResponse(**response_data), RiskDefinitionCheckResponse
    )
    assert (
        response_data["suggestion"]
        == "Consider adding buffer time to the project schedule."
    )
    assert (
...
def test_category_identification(mock_execute_query):
    data = CategoriesIdentificationRequest(
        text="This is a sample text to identify categories."
    )
